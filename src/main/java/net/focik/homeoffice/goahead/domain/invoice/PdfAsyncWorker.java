...
public class PdfAsyncWorker {
    ...
    public void processJobAsync(String jobId, List<Integer> invoiceIds) {
        AsyncContext.setJobType("PDF_INVOICE_GENERATION");
        try {
            ...
            log.info("Starting PDF generation job: {} for {} invoices", jobId, invoiceIds.size());
            ...
            for (Integer id : invoiceIds) {
                try {
                    ...
                    if (s3Url == null) {
                        failed++;
                        job.getErrors().add(new AsyncTaskError(String.valueOf(id), "Nie udało się wygenerować PDF dla faktury"));
                    } else {
                        processed++;
                    }
